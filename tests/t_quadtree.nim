...
suite "Quadtrees should":
    ...
    test "Adding to a tree without subdividing":
        ...
        tree.insert( box1 )
        tree.insert( box2 )
        check(tree.bounds.get == (y: 0, x: 0, size: 32))
        check( tree.fetch(3, 3) == @[ box1, box2 ] )
    ...
    test "Subdividing a leaf node after it gets full":
        ...
        tree.insert( box1 )
        tree.insert( box2 )
        check(tree.bounds.get == (y: -1, x: -1, size: 16))
        check( tree.fetch(1, 1) == @[ box1 ] )
        check( tree.fetch(11, 1) == @[ box2 ] )

    test "Return the bouding box of a tree":
        var tree = newQuadtree[Box]()
        check( tree.bounds == none(Square) )
        tree.insert( (x: 10, y: 2, width: 2, height: 2) )
        check(tree.bounds.get == (y: 1, x: 9, size: 8))

    test "Expand the boundaries when adding an outside right element":
        var tree = newQuadtree[Box]()
        tree.insert( (x: 3, y: 5, width: 2, height: 2) )
        check(tree.bounds.get == (y: 4, x: 2, size: 8))

        tree.insert( (x: 10, y: 5, width: 2, height: 2) )
        check(tree.bounds.get == (y: -4, x: -6, size: 32))
    ...
    test "Expand the until it fits the new element":
        ...
        check(tree.bounds.get == (y: -174756, x: -174758, size: 524288))
    ...
    test "Fetching from an empty quadrant":
        ...
        check(tree.bounds.get == (y: 0, x: 0, size: 16))
        check(tree.fetch(9, 0) == empty)
        check(tree.fetch(10, 10) == empty)
        check(tree.fetch(0, 11) == empty)

    test "Fetching from outside a tree with a single node tree":
        var tree = newQuadtree[Box](maxInQuadrant = 1)
        tree.insert( (x: 1, y: 1, width: 3, height: 3) )
        check(tree.bounds.get == (y: 0, x: 0, size: 16))
        check(tree.fetch(-5, 5) == empty)
        check(tree.fetch(5, -5) == empty)
        check(tree.fetch(5, 20) == empty)
        check(tree.fetch(20, 5) == empty)
    ...
    test "Delete elements from a tree":
        ...
        check(tree.fetch(2, 2) == empty)
        check(tree.fetch(9, 9) == @[ (x: 9, y: 9, width: 3, height: 3) ])

        tree.delete( (x: 9, y: 9, width: 3, height: 3) )
        check(tree.fetch(9, 9) == empty)
